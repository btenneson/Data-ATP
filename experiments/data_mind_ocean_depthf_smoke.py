...
def run_case(version: str, problem: Path, solver_result: dict[str, Any], trace,
    ...
    # Self-awareness is part of both architectures being smoke-tested.
    ...
    if version == "1.1":
        ...
        # Capability smoke: every one of the eight agents gets a turn on the
        # same shared deliberation bus before a BANK proposal exists.
        for agent in AGENTS:
        ...
        # 1.0 has the same eight role identities available, but this comparison
        # does not grant the new unrestricted shared pre-BANK bus.
        for agent in AGENTS:
    ...
    # Sovereign verifier gate. The harness accepts only the independently
    # checked certificate status emitted by the frozen Depths-F control.
...
def main() -> None:
    ...
    if version == "1.1":
        ...
        # Assert the comparison arm did not accidentally receive 1.1-only hooks.
        forbidden = {
