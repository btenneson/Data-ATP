...
class SecurityFlag(StrEnum):
    BENIGN = "benign"
    MALIGNANCY_LOW = "malignancy_low"
    MALIGNANCY_HIGH = "malignancy_high"
    MALIGNANCY_CRITICAL = "malignancy_critical"
...
class TransactionLog:
    """Append-only SHA-256 hash chain with mandatory 2.9 security flags.

    Existing pre-2.9 records remain byte-for-byte immutable and continue to
    verify. Every newly appended transaction receives ``_security_flag`` in its
    hashed payload, so classification is itself tamper-evident.
    """
    ...
    @staticmethod
    def _derive_security_flag(event_type: EventType, payload: dict[str, Any]) -> SecurityFlag:
        explicit = payload.get("_security_flag")
        if explicit is not None:
            return SecurityFlag(str(explicit))

        security_class = str(payload.get("security_class", "")).lower()
        sentinel_decision = str(payload.get("decision", "")).lower()
        risk = float(payload.get("risk_score", payload.get("sentinel_risk_score", 0.0)) or 0.0)

        if security_class.endswith("prohibited") or "block" in sentinel_decision:
            return SecurityFlag.MALIGNANCY_CRITICAL
        if security_class.endswith("high_risk") or "require_human" in sentinel_decision or risk >= 0.70:
            return SecurityFlag.MALIGNANCY_HIGH
        if security_class.endswith("dual_use") or "quarantine" in sentinel_decision or risk >= 0.25:
            return SecurityFlag.MALIGNANCY_LOW
        if event_type is EventType.SENTINEL_BLOCKED:
            return SecurityFlag.MALIGNANCY_HIGH
        return SecurityFlag.BENIGN
    ...
    def append(self, event_type: EventType, payload: dict[str, Any]) -> Transaction:
        ...
        classified_payload["_security_flag"] = self._derive_security_flag(event_type, classified_payload)
        sequence = len(self._items)
        timestamp = datetime.now(timezone.utc).isoformat()
        previous_hash = self.last_digest
        digest = self._digest(sequence, event_type, classified_payload, timestamp, previous_hash)
        tx = Transaction(
            sequence=sequence,
